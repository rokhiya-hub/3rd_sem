...
-- Finding out how many employees are working
-- in each department
SELECT department, COUNT(*) AS emp_count
FROM employee
GROUP BY department;
-- Find out how many female, male employees
-- are working in the organization
SELECT gender, COUNT(*) as emp_count
FROM employee
GROUP BY gender;
-- GROUP BY
-- All the columns used in select statement
-- of a GROUP BY query must also
-- be used in GROUP BY clause or must be aggregated
SELECT department, MAX(salary), MIN(salary), SUM(salary)
FROM employee
GROUP BY department;

SELECT gender, MAX(age), SUM(salary), MIN(date_of_joining)
FROM employee
GROUP BY gender;

-- WHERE + GROUP BY
-- I want to find out how many employees are there
-- from each department that are earning more than 
-- 60k
...
-- HAVING
-- HAVING is like what WHERE does to individual rows
