...
--Find out for each question, how many submissions are made(display question_id and total_submissions)
SELECT question_id,COUNT(*) AS total_submissions
FROM submissions
GROUP BY question_id
ORDER BY question_id;

--Find out for each question how many successful submissions are made
SELECT question_id,COUNT(*) AS successful_submissions 
FROM submissions 
WHERE status = 1
GROUP BY question_id
ORDER BY question_id;

--Find out for each question how many submissions are made along with question_name, rating, total_submissions
SELECT q.q_id,q.q_name,q.rating,COUNT(*) AS total_submissions
FROM
submissions s
JOIN
question q
...
--Find out how many total submissions are made for each contest(display only contest_id and total_submissions)
SELECT q.contest_id,COUNT(*) AS total_submissions
FROM 
submissions s
JOIN
question q
...
--Find out how many total submissions are made for each contest(display contest_name, contest_id and total_submissions)
SELECT c.c_id,c.c_name,COUNT(*) AS total_submissions
FROM 
submissions s
JOIN
question q
...
--Find out for each contest, for each question, how manu submissions are made(contest_id,questioo_id,n0.of_submissions)
SELECT q.contest_id,q.q_id,q.q_name,COUNT(*) AS total_submissions
FROM 
submissions s
JOIN
question q
...
--Find out how many total submission are made for each contest contest_name,contest_date and total submissions
SELECT c_name,c_date, count(*) AS total_submissions
from questions q join submissions s
ON s.question_id = q.q_id;

-- GROUP BY c.c_name,c.c_date;

--Find out how many submissions are made for each type of 


--Find out how many submissions are made for each user form each contest(u_name, c_name,no_of_sub)
