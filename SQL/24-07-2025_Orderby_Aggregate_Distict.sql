...
-- Select name, department, email of all the Female employees
-- belong to SW deparment with
...
-- Select id, name, age, years_of_service_left for all the employees. 
...
-- DISTINCT 
-- Is used to get distinct values from
-- single column or multiple columns
-- How it works on single column
...
-- Using DISTINCT on multiple columnns
-- makes sure the combination of columns
-- used is never repeated interms of data
...
-- IN/NOT IN
-- Used to check wheter a value is a part of 
-- collection of values in comparison
...
-- iNCLUSION
SELECT * FROM employee
WHERE department IN ('SW', 'HR');

-- Exclusion
SELECT * FROM employee
WHERE department NOT IN ('SW', 'HR');


-- ORDER BY
-- ORDER BY used to order the data
-- in either ASCENDING or DESCENDING
-- based on 1 column or multiple columns
-- By default order by orders the data
-- in ASCending order only
-- DESCending order requires an explicit mention
-- of DESC keyword
-- ASC NULLs come last
-- DESC NULLs come first
...
-- usage of multiple columns in order by
-- leads to break the ties
SELECT id, name, salary, age, department
FROM employee ORDER BY salary DESC, age, department DESC;

-- Aggregate Fuctions, 
-- Used to find the MAX(), MIN(), AVG(), SUM(), COUNT()
-- of column data
-- SUM() - Used to find the sum of all the values
-- present under a column, works on numeric only
SELECT SUM(salary) FROM employee;
SELECT SUM(age) FROM employee;

-- What is the total salary of all 
-- empmloyees from HR
SELECT SUM(salary) FROM employee
WHERE department = 'HR';
...
-- COUNT()
-- count is used to give how manny
-- Finnd out how many employee
-- are workig under HR
SELECT COUNT(department) FROM employee
WHERE department = 'HR';

-- uSIG COUNT with column name
-- doesnt count NULL
SELECT COUNT(department) FROM employee;
...
-- Find out how many female employees
-- are working in operations deparment
SELECT COUNT(*) FROM employee
WHERE department = 'Operations' 
AND gender = 'Female';

-- Find out for how many employees
-- the email is missing
SELECT COUNT(*) FROM employee
WHERE email IS NULL;

-- Find out how many employees joined in 
-- Testing department after 12-07-2019
SELECT COUNT(*) FROM employee
WHERE department = 'Testing' AND
date_of_joining > TO_DATE('12-07-2019', 'DD-MM-YYYY');

SELECT COUNT(gender), COUNT(age) FROM
employee;
...
-- fiding maximum name (based on alphabet) from HR
